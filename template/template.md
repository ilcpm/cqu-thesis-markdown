...
**对标题标记**：在标题后面加上`{#jcyy}`这样的标记（这里对上面的一级标题进行了标注）
...
由于附录的字号比正文小，所以这里测试一下在正文中的内联代码字号是否正常👉`内联代码code code`👈可见内联代码的字号并未跟随改变，和前文（见[@page-内联代码字号问题]页）中提到的脚注中的代码一样。
